			...
			Endpoints: []apiEndpoint{
				{
					...
					Auth:     "admin",
					Response: `{"configs":{"app.name":{"name":"app.name","type":"string","value":"AgentStore","description":"Application name","isSystem":true,"options":""},...}}`,
				},
				{
					Method:   "POST",
					Path:     "/api/admin/config",
				...
				},
				{
					...
					Response: `{"name":"app.name","type":"string","value":"AgentStore","description":"Application name","isSystem":true,"options":""}`,
				},
				{
					Method:  "PUT",
					Path:    "/api/admin/config/{name}",
...
<title>AgentStore API Reference</title>
<style>
...
<body>
`)
	sb.WriteString(fmt.Sprintf(`<h1>AgentStore API Reference</h1><p class="subtitle">Version %s</p>`, html.EscapeString(version.Current)))
...
func DocsMarkdown(w http.ResponseWriter, r *http.Request) {
	...
	sb.WriteString(fmt.Sprintf("# AgentStore API Reference\n\n**Version:** %s\n\n", version.Current))
