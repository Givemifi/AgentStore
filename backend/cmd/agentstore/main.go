...
func printUsage() {
	fmt.Println(`agentstore - AgentStore system administration tool

Usage:
  agentstore <command> [flags] [--json]
...
Examples:
  agentstore setup
  agentstore start --backend
  agentstore stop

  agentstore logs --severity critical,high --tail 100
  agentstore logs --follow --category security
  agentstore logs --from 24h
  agentstore users list --json
  agentstore users get --email admin@example.com
  agentstore users suspend --email bad@example.com
  agentstore tenants list
  agentstore tenants get root

  agentstore health
  agentstore stats --json
  agentstore financial summary
  agentstore financial transactions --type subscription --limit 50
  agentstore financial metrics --days 30
  agentstore doctor
  agentstore db stats

  agentstore config list
  agentstore config get log.min_level
  agentstore config set log.min_level high
  agentstore config reset log.min_level
  agentstore change-password --email admin@example.com

  AGENTSTORE_URL=http://localhost:3000 AGENTSTORE_API_KEY=lsk_xxx agentstore mcp`)
}
...
func cmdSetup() {
	...
	if err == nil && sys.Initialized {
		...
		fmt.Println()
		fmt.Println("If you need to change a user's password, use: agentstore change-password --email <email>")
		os.Exit(0)
	}
...
func cmdChangePassword() {
	...
	if *emailFlag == "" {
		fmt.Fprintln(os.Stderr, "Usage: agentstore change-password --email <email>")
		os.Exit(1)
	}
...
func cmdSendMessage() {
	...
	if *emailFlag == "" || *messageFlag == "" {
		fmt.Fprintln(os.Stderr, "Usage: agentstore send-message --email <email> --message \"Your message here\"")
...
func cmdConfig() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, `Usage: agentstore config <subcommand>
...
Subcommands:
  ...
  reset <name>      Reset a variable to its default value`)
	...
	case "get":
		if len(os.Args) < 4 {
			fmt.Fprintln(os.Stderr, "Usage: agentstore config get <name>")
			os.Exit(1)
		}
		cmdConfigGet(os.Args[3])
	case "set":
		if len(os.Args) < 5 {
			fmt.Fprintln(os.Stderr, "Usage: agentstore config set <name> <value>")
			os.Exit(1)
		}
		cmdConfigSet(os.Args[3], strings.Join(os.Args[4:], " "))
	case "reset":
		if len(os.Args) < 4 {
			fmt.Fprintln(os.Stderr, "Usage: agentstore config reset <name>")
			os.Exit(1)
		}
...
func cmdTransferRootOwner() {
	...
	if *emailFlag == "" {
		fmt.Fprintln(os.Stderr, "Usage: agentstore transfer-root-owner --email <new-owner-email>")
		os.Exit(1)
	}
...
func cmdVersion() {
	fmt.Printf("AgentStore v%s (binary)\n", version.Current)
...
func cmdStatus() {
	...
	if err != nil || !sys.Initialized {
		...
		fmt.Println()
		fmt.Println("Run 'agentstore setup' to initialize the system.")
		return
	}
