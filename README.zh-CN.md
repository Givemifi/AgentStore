...
- 可分享对话链接
- **知识库（RAG）**：管理员为每个 Agent 上传领域文档/文本/问答对；内容自动分块、嵌入向量、对话时余弦相似度检索注入，检索失败降级不阻断聊天
- **数据标注工作台**（`/admin/annotations`）：审阅用户 👍/👎 反馈、打质量分（1–5）、问题分类标注（事实错误/幻觉等）、撰写理想回答，一键沉淀为知识库或导出 SFT JSONL
- **对话管理**：删除会话（级联清理消息与标注）、内联重命名、一键复制回答、原地重新生成
