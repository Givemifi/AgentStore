...
- 会話共有リンク
- **エージェント別ナレッジベース（RAG）**：管理者がドキュメント・テキスト・Q&A を登録；チャンク分割・ベクトル埋め込み・コサイン類似度検索でシステムプロンプトへ自動注入。検索失敗時はグレースフルデグレード
- **データアノテーションワークベンチ**（`/admin/annotations`）：ユーザーの 👍/👎 フィードバックをレビュー、品質スコア（1–5）・問題タグ付け・理想回答の記述、ワンクリックでナレッジベースへ昇格または SFT JSONL エクスポート
- **会話管理**：会話削除（メッセージ・アノテーションをカスケード削除）、インラインリネーム、回答コピー、その場での再生成
